...
### 13.1 固定场景（20 个，见 `eval/scenarios.json`）

最初只设计了 10 个场景（覆盖六类核心能力），后来发现两个问题促使扩充到 20 个：
一是样本量太小、"100% 通过"说服力不够；二是原来的断言只检查 `status` 和 `request` 字段，
没有真正校验"选出来的航班对不对"，往返场景甚至根本没有真的去搜索回程航班
（`trip_duration_days` 解析出来了但从没被用过——这是扩充场景时才发现的真实 bug，已在
`src/nodes.py` 里修复：`search_flights_node` 现在会在 `trip_duration_days` 存在时，
用"出发日期 + 行程天数"算出回程日期并真正搜索回程航班，`rank_and_tier_node` 按档位配对
计算往返合计总价）。
...
| 8 | 无符合条件结果（目的地完全没有数据） | 走 `explain_failure` 分支，给出清晰说明 |
...
| 指标 | 定义 | 测量方法 |
|---|---|---|
| 参数提取正确率 | 20 个场景中，`parse_input` 输出的结构化字段与预期一致的比例 | `run_eval.py` 的 `field_matches` 逐场景校验 `expected_fields` |
| 工具调用成功率 | 在“预期会走到 `search_flights`”的场景（即 `expected_status != clarification`）里，实际状态也确实是 `results`/`error` 的比例 | 分母只统计需要调用工具的场景，不含追问类场景 |
| 端到端任务完成率 | 状态匹配预期 **且** 字段匹配预期 **且**（如果场景定义了 `expected_results`）实际选出的航班/档位也匹配预期的比例 | `status_ok and fields_ok and results_ok`，`results_ok` 校验的是真实业务结果，不只是状态码 |
| 平均响应时间 + 平均 Token 成本 | 20 次请求的平均耗时（`time.perf_counter`）与平均 Token 用量（来自 LLM 响应的 usage 字段） | `run_eval.py` 汇总输出 |
...
- 无用户偏好记忆，无持久化存储。
- 样本量（20 场景）仍不足以支撑 P95 等统计量，因此不纳入本轮指标。
...
- **状态合并无法显式"清空"一个已知字段**：`parse_input` 的合并规则是"本轮没提到就保留旧值"，
  ...
  列入 Roadmap，不建议在简历/面试里说这个功能已经支持。
