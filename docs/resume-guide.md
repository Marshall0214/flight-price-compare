...
- **评测过程中实际发现了什么问题？（很值得主动讲的一段）** 一共发现并修了 4 处真实 bug、记录了几处真实但不修的限制，都不是靠代码审查发现的，是靠评测/手动跑通发现的：
